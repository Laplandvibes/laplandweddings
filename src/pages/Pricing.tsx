...
const breakdown: CostRow[] = [
  ...
  },
  {
    ...
    note: {
      ...
      es: 'Una excursión de 2,5 horas, con 45 minutos conduciendo su propio trineo, precio de adulto. Los niños pagan menos.',
...
const quotationOnly: Array<Localized<string>> = [
  {
    ...
    'zh-CN': '婚礼策划费用', ko: '웨딩 플래너 비용', fr: 'Honoraires de l’organisateur',
...
type PKey =
  ...
  | 'elopementCrossRef' | 'elopementCrossRefLink'
  | 'floorNote'
...
const P: Record<PKey, Localized<string>> = {
  ...
  d3Body: { en: 'A glass igloo for one night and a cabin for the rest is a different figure from three nights under glass. Husky, reindeer and snowmobile outings are priced per person, per trip.', fi: 'Yksi yö lasi-iglussa ja loput mökissä on eri summa kuin kolme yötä lasikaton alla. Husky-, poro- ja kelkkaretket hinnoitellaan per henkilö ja per retki.', de: 'Eine Nacht im Glasiglu und der Rest in einer Hütte ist eine andere Zahl als drei Nächte unter Glas. Husky-, Rentier- und Schneemobiltouren werden pro Person und pro Ausflug berechnet.', ja: 'グラスイグルーに1泊して残りをコテージで過ごす場合と、3泊すべてをガラス屋根の下で過ごす場合とでは金額が異なります。ハスキー、トナカイ、スノーモービルの各ツアーは一人あたり・一回あたりの料金です。', es: 'Una noche en iglú de cristal y el resto en cabaña es una cifra distinta a tres noches bajo el cristal. Las excursiones de huskies, renos y motos de nieve se cobran por persona y por salida.', 'pt-BR': 'Uma noite em iglu de vidro e o resto em cabana dá um número diferente de três noites sob o vidro. Passeios de husky, rena e moto de neve são cobrados por pessoa e por saída.', 'zh-CN': '在玻璃冰屋住一晚、其余住木屋，与三晚都住在玻璃屋顶下，是两个不同的数字。哈士奇、驯鹿和雪地摩托行程按人次计价。', ko: '유리 이글루에서 하룻밤을 보내고 나머지를 캐빈에서 지내는 것과 사흘 내내 유리 지붕 아래에서 묵는 것은 금액이 다릅니다. 허스키, 순록, 스노모빌 투어는 1인당, 1회당 요금입니다.', fr: 'Une nuit en igloo de verre et le reste en chalet, ce n’est pas le même chiffre que trois nuits sous le verre. Les sorties husky, renne et motoneige se facturent par personne et par excursion.', it: 'Una notte in igloo di vetro e il resto in chalet è una cifra diversa da tre notti sotto il vetro. Le escursioni con husky, renne e motoslitte si pagano a persona e a uscita.', nl: 'Eén nacht in een glazen iglo en de rest in een hut is een ander bedrag dan drie nachten onder glas. Husky-, rendier- en sneeuwscootertochten rekenen per persoon en per tocht.', sv: 'En natt i glasigloo och resten i stuga är en annan siffra än tre nätter under glas. Husky-, ren- och skoterturer prissätts per person och per tur.' },
  seoTitle: { en: 'Lapland wedding with guests: our EUR 5,000 minimum | LaplandWeddings', fi: 'Häät vieraiden kanssa: pienin budjetti 5 000 € | LaplandWeddings', de: 'Hochzeit mit Gästen in Lappland: Mindestbudget 5.000 € | LaplandWeddings', ja: 'ゲストを招く結婚式：最小のご予算5,000ユーロ | LaplandWeddings', es: 'Boda con invitados en Laponia: presupuesto mínimo 5 000 € | LaplandWeddings', 'pt-BR': 'Casamento com convidados na Lapônia: orçamento mínimo € 5.000 | LaplandWeddings', 'zh-CN': '有宾客的拉普兰婚礼：最低预算 5,000 欧元 | LaplandWeddings', ko: '하객이 있는 라플란드 결혼식: 최소 예산 5,000유로 | LaplandWeddings', fr: 'Mariage avec invités en Laponie : budget minimum 5 000 € | LaplandWeddings', it: 'Matrimonio con ospiti in Lapponia: budget minimo 5.000 € | LaplandWeddings', nl: 'Bruiloft met gasten in Lapland: minimumbudget € 5.000 | LaplandWeddings', sv: 'Bröllop med gäster i Lappland: minsta budget 5 000 € | LaplandWeddings' },
  seoDesc: { en: 'What does a wedding in Lapland cost? EUR 5,000 is the smallest budget at which we pass an enquiry on to a planner, and it applies to weddings with guests: a published elopement package for two starts at about EUR 1,600. What the sum covers, what pushes it up, and price ranges per item.', fi: 'Mitä häät Lapissa maksavat? 5 000 € on pienin budjetti, jolla välitämme toimeksiannon suunnittelijalle, ja se koskee häitä, joissa on vieraita: kahden hengen julkaistu elopement-paketti alkaa noin 1 600 eurosta. Mitä summa kattaa, mikä sitä nostaa ja hintahaarukat osa kerrallaan.', de: 'Was kostet eine Hochzeit in Lappland? 5.000 € ist das kleinste Budget, mit dem wir eine Anfrage an einen Planer weitergeben, und das gilt für Hochzeiten mit Gästen. Ein veröffentlichtes Elopement-Paket zu zweit beginnt bei rund 1.600 €. Was enthalten ist, was die Summe erhöht, und Preisspannen pro Posten.', ja: 'ラップランドの結婚式はいくら？5,000ユーロは、私たちがプランナーにお繋ぎする最小のご予算で、ゲストを招く結婚式が対象です。二人だけのエロープメントの公表パッケージは約1,600ユーロから。内訳、費用が上がる要因、項目ごとの価格帯をご紹介します。', es: '¿Cuánto cuesta una boda en Laponia? 5 000 € es el presupuesto mínimo con el que trasladamos una consulta a un organizador, y se refiere a bodas con invitados: un paquete de elopement para dos publicado parte de unos 1 600 €. Qué cubre, qué lo eleva y rangos de precio por partida.', 'pt-BR': 'Quanto custa um casamento na Lapônia? € 5.000 é o menor orçamento com que encaminhamos um pedido a um organizador, e vale para casamentos com convidados: um pacote de elopement a dois publicado parte de cerca de € 1.600. O que cobre, o que aumenta o valor e faixas de preço por item.', 'zh-CN': '在拉普兰办婚礼要花多少钱？5,000 欧元是我们将咨询转交给策划师的最低预算，适用于有宾客的婚礼；两人私奔婚礼的公开套餐约 1,600 欧元起。本页说明这笔预算涵盖什么、哪些因素会推高费用，以及各项开支的价格区间。', ko: '라플란드 결혼식 비용은 얼마일까요? 5,000유로는 저희가 플래너에게 문의를 전달하는 최소 예산이며, 하객이 있는 결혼식에 해당합니다. 두 사람만의 엘로프먼트 공개 패키지는 약 1,600유로부터입니다. 예산 구성, 금액을 높이는 요인, 항목별 가격대를 정리했습니다.', fr: 'Combien coûte un mariage en Laponie ? 5 000 € est le budget minimum à partir duquel nous transmettons une demande à un organisateur, et cela vaut pour les mariages avec invités : un forfait elopement publié à deux démarre autour de 1 600 €. Ce qu’il couvre, ce qui le fait grimper, et les fourchettes par poste.', it: 'Quanto costa un matrimonio in Lapponia? 5.000 € è il budget minimo con cui trasmettiamo una richiesta a un planner e riguarda i matrimoni con ospiti: un pacchetto elopement pubblicato per due parte da circa 1.600 €. Cosa comprende, cosa lo fa salire e le fasce di prezzo per ogni voce.', nl: 'Wat kost een bruiloft in Lapland? € 5.000 is het kleinste budget waarmee wij een aanvraag aan een planner doorgeven, en dat geldt voor bruiloften met gasten: een gepubliceerd elopementpakket voor twee begint rond € 1.600. Wat het dekt, wat het omhoog duwt en prijsranges per onderdeel.', sv: 'Vad kostar ett bröllop i Lappland? 5 000 € är den minsta budget som vi för en förfrågan vidare till en planerare med, och den gäller bröllop med gäster: ett publicerat rymningspaket för två börjar kring 1 600 €. Vad den täcker, vad som driver upp den och prisintervall per post.' },
  heroEyebrow: {
  ...
  heroSubtitle: { en: 'EUR 5,000 is the smallest budget at which we pass an enquiry on to a planner. It applies to a wedding with guests; a ceremony for two costs far less. Below: what makes up that sum, and what pushes it higher.', fi: '5 000 € on pienin budjetti, jolla välitämme toimeksiannon suunnittelijalle. Se koskee häitä, joissa on vieraita. Kahden hengen vihkiminen maksaa selvästi vähemmän. Alla mistä summa muodostuu ja mikä sitä nostaa.', de: '5.000 € ist das kleinste Budget, mit dem wir eine Anfrage an einen Planer weitergeben. Es gilt für eine Hochzeit mit Gästen; eine Trauung zu zweit kostet deutlich weniger. Unten: woraus sich die Summe zusammensetzt und was sie erhöht.', ja: '5,000ユーロは、私たちがプランナーにお問い合わせをお繋ぎする最小のご予算です。これはゲストを招く結婚式の目安で、二人だけの挙式はこれよりずっと低く収まります。以下では、その金額の内訳と、費用が上がる要因をご説明します。', es: '5 000 € es el presupuesto mínimo con el que trasladamos una consulta a un organizador. Se refiere a una boda con invitados; una ceremonia para dos cuesta bastante menos. Abajo: de qué se compone esa cifra y qué la eleva.', 'pt-BR': '€ 5.000 é o menor orçamento com que encaminhamos um pedido a um organizador. Vale para um casamento com convidados; uma cerimônia a dois custa bem menos. Abaixo: do que se compõe essa quantia e o que a aumenta.', 'zh-CN': '5,000 欧元是我们将咨询转交给策划师的最低预算，适用于有宾客的婚礼；两人的仪式要便宜得多。以下说明这笔预算的构成，以及哪些因素会推高费用。', ko: '5,000유로는 저희가 플래너에게 문의를 전달하는 최소 예산입니다. 하객이 있는 결혼식 기준이며, 두 사람만의 예식은 훨씬 적게 듭니다. 아래에서 그 금액의 구성과 비용을 높이는 요인을 설명합니다.', fr: '5 000 € est le budget minimum à partir duquel nous transmettons une demande à un organisateur. Cela concerne un mariage avec invités ; une cérémonie à deux coûte nettement moins. Ci-dessous : ce qui compose cette somme et ce qui la fait grimper.', it: '5.000 € è il budget minimo con cui trasmettiamo una richiesta a un planner. Riguarda un matrimonio con ospiti; una cerimonia in due costa molto meno. Qui sotto: da cosa è composta quella cifra e cosa la fa salire.', nl: '€ 5.000 is het kleinste budget waarmee wij een aanvraag aan een planner doorgeven. Dat geldt voor een bruiloft met gasten; een ceremonie met z’n tweeën kost veel minder. Hieronder: waaruit dat bedrag bestaat en wat het omhoog duwt.', sv: '5 000 € är den minsta budget som vi för en förfrågan vidare till en planerare med. Den gäller ett bröllop med gäster; en vigsel för två kostar betydligt mindre. Nedan: vad summan består av och vad som driver upp den.' },
  heroImageAlt: {
    en: 'The Luiro river at Savukoski on a summer night, from the air',
    fi: 'Luirojoki Savukoskella kesäyönä ilmasta kuvattuna',
    de: 'Der Fluss Luiro bei Savukoski in einer Sommernacht aus der Luft',
    ja: '夏の夜、空から見たサヴコスキのルイロ川',
    es: 'El río Luiro en Savukoski en una noche de verano, desde el aire',
    'pt-BR': 'O rio Luiro em Savukoski numa noite de verão, visto do ar',
    'zh-CN': '夏夜空中俯瞰萨武科斯基的卢伊罗河',
    ko: '여름밤 하늘에서 본 사부코스키의 루이로강',
    fr: 'La rivière Luiro à Savukoski par une nuit d’été, vue du ciel',
    it: 'Il fiume Luiro a Savukoski in una notte d’estate, dall’alto',
    nl: 'De rivier Luiro bij Savukoski op een zomernacht, vanuit de lucht',
    sv: 'Älven Luiro vid Savukoski en sommarnatt, från luften',
  },
  ...
  s1Title: { en: 'With guests, the smallest budget we pass on is EUR 5,000', fi: 'Vieraiden kanssa pienin välitettävä budjetti on 5 000 €', de: 'Mit Gästen: das kleinste weitergegebene Budget sind 5.000 €', ja: 'ゲストを招く場合、お繋ぎできる最小のご予算は5,000ユーロ', es: 'Con invitados, el presupuesto mínimo que trasladamos son 5 000 €', 'pt-BR': 'Com convidados, o menor orçamento que encaminhamos é € 5.000', 'zh-CN': '有宾客时，我们转交的最低预算为 5,000 欧元', ko: '하객이 있을 때 전달하는 최소 예산은 5,000유로', fr: 'Avec des invités, le budget minimum que nous transmettons est de 5 000 €', it: 'Con ospiti, il budget minimo che trasmettiamo è di 5.000 €', nl: 'Met gasten is het kleinste budget dat wij doorgeven € 5.000', sv: 'Med gäster är den minsta budget vi för vidare 5 000 €' },
  whereMoneyGoes: {
  ...
  ctaEyebrow: {
    ...
    es: 'Cuando conoce su presupuesto', 'pt-BR': 'Quando você já sabe seu orçamento',
  ...
  ctaTitle: {
    ...
    es: 'Reciba 1–3 presupuestos y compare con calma',
  ...
  ctaBody: {
    en: 'One form, 1–7 days, 1–3 personalised proposals from wedding planners in Lapland. Free, no commitment. You decide who to continue with.',
    fi: 'Yhdellä lomakkeella saat 1–7 päivän sisällä 1–3 räätälöityä tarjousta Lapin hääsuunnittelijoilta. Maksuton, ei sitoumusta. Sinä päätät kenen kanssa jatkat.',
    de: 'Ein Formular, 1–7 Tage, 1–3 individuelle Angebote von Hochzeitsplanern in Lappland. Kostenlos, unverbindlich. Sie entscheiden, mit wem Sie weitermachen.',
    ja: 'フォームを1つ、1〜7日で、ラップランドのウェディングプランナーから1〜3つのオーダーメイド提案。無料・無拘束。誰と進めるかはあなた次第です。',
    es: 'Un formulario, de 1 a 7 días, 1–3 propuestas personalizadas de organizadores de bodas de Laponia. Gratis, sin compromiso: usted decide con quién seguir.',
    'pt-BR': 'Um formulário, de 1 a 7 dias, 1–3 propostas personalizadas de organizadores de casamento da Lapônia. Grátis, sem compromisso. Você decide com quem continuar.',
    'zh-CN': '一份表单，1–7 天，来自拉普兰婚礼策划师的 1–3 份个性化方案。免费、无约束，由你决定与谁继续。',
    ko: '양식 하나, 1~7일, 라플란드 웨딩 플래너의 맞춤 제안 1~3건. 무료, 부담 없음. 누구와 진행할지는 당신이 결정합니다.',
    fr: 'Un formulaire, 1 à 7 jours, 1 à 3 propositions personnalisées d’organisateurs de mariage de Laponie. Gratuit, sans engagement. Vous décidez avec qui continuer.',
    it: 'Un modulo, 1–7 giorni, 1–3 proposte personalizzate da wedding planner della Lapponia. Gratis, senza impegno. Decide Lei con chi proseguire.',
    nl: 'Eén formulier, 1–7 dagen, 1–3 persoonlijke voorstellen van weddingplanners in Lapland. Gratis, vrijblijvend. U bepaalt met wie u verdergaat.', sv: 'Ett formulär, 1–7 dagar, 1–3 personliga offerter från bröllopsplanerare i Lappland. Gratis och utan förbindelse. Du bestämmer vem du går vidare med.',
  },
  ctaButton: {
    en: 'Start the 5-minute form', fi: 'Aloita 5 minuutin lomake',
    de: 'Das 5-Minuten-Formular starten', ja: '5分のフォームを始める',
    es: 'Empiece el formulario de 5 minutos', 'pt-BR': 'Comece o formulário de 5 minutos',
    'zh-CN': '开始 5 分钟表单', ko: '5분 양식 시작하기',
    fr: 'Commencer le formulaire de 5 minutes', it: 'Inizi il modulo di 5 minuti',
  ...
  s4Title: {
    ...
    es: 'O rellene el formulario aquí mismo', 'pt-BR': 'Ou preencha o formulário aqui mesmo',
    'zh-CN': '或在此直接填写表单', ko: '또는 여기에서 바로 양식을 작성하세요',
    fr: 'Ou remplissez le formulaire ici même', it: 'Oppure compili il modulo qui',
    nl: 'Of vul het formulier hier direct in', sv: 'Eller fyll i formuläret här',
  },
  s4Subtitle: {
    en: 'Fill in the budget field so we can match you with planners at the right price level.',
    ...
    es: 'Indique el campo de presupuesto para que podamos emparejarle con organizadores del nivel de precio adecuado.',
    ...
    fr: 'Renseignez le champ budget pour que nous puissions vous orienter vers des organisateurs au bon niveau de prix.',
